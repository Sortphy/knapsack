    ...
    // New algorithm buttons (make sure these exist in your HTML)
    ...
    const approxBtn = document.getElementById('approx-btn');
    const fptasBtn = document.getElementById('fptas-btn');
    ...
    function initialize() {
      ...
      solveBtn.addEventListener('click', solveKnapsack); // Brute force already provided
      dpBtn && dpBtn.addEventListener('click', solveKnapsackDP);
      greedyBtn && greedyBtn.addEventListener('click', solveKnapsackGreedy);
      bnbBtn && bnbBtn.addEventListener('click', solveKnapsackBnB);
      gaBtn && gaBtn.addEventListener('click', solveKnapsackGA);
      saBtn && saBtn.addEventListener('click', solveKnapsackSA);
      approxBtn && approxBtn.addEventListener('click', solveKnapsackApprox);
      fptasBtn && fptasBtn.addEventListener('click', solveKnapsackFPTAS);
    ...
    function highlightSelectedItems() {
      ...
      if (optimalSolution && optimalSolution.bestCombination) {
        document.querySelectorAll('.item').forEach(el => el.classList.remove('in-optimal'));
        optimalSolution.bestCombination.forEach(item => {
          const itemElement = document.querySelector(`.item[data-id="${item.id}"]`);
    ...
    function solveKnapsack() {
      ...
      switch(algorithm) {
        case 'bruteforce':
          solution = bruteForceKnapsack(items, capacity);
          break;
        case 'dp':
          solution = solveKnapsackDP();
          return; // DP handles its own display
        case 'dp_recursive':
          solution = solveKnapsackDPRecursive();
          return; // Recursive DP handles its own display
        case 'greedy':
          solution = solveKnapsackGreedy();
          return; // Greedy handles its own display
        case 'bnb':
          solution = solveKnapsackBnB();
          return; // BnB handles its own display
        case 'ga':
          solution = solveKnapsackGA();
          return; // GA handles its own display
        case 'sa':
          solution = solveKnapsackSA();
          return; // SA handles its own display
        case 'approx':
          solution = solveKnapsackApprox();
          return; // Approx handles its own display
        case 'fptas':
          solution = solveKnapsackFPTAS();
          return; // FPTAS handles its own display
        default:
          solution = bruteForceKnapsack(items, capacity);
      }
      ...
      totalWeightSpan.textContent = calculateTotalWeight(optimalSolution.bestCombination);
      stepsTakenSpan.textContent = optimalSolution.steps || '-';
      document.getElementById('execution-time').textContent = `${executionTime.toFixed(2)} ms`;
      resultPanel.style.display = 'block';
      
      if (optimalSolution.bestCombination.length === 0) {
        optimalSolutionDiv.innerHTML = '<p>Nenhuma combinação válida encontrada.</p>';
      } else {
        optimalSolutionDiv.innerHTML = `
          <p>A solução ótima inclui os seguintes itens:</p>
          <ul>
            ${optimalSolution.bestCombination.map(item => `<li>Item ${item.id}: Valor = $${item.value}, Peso = ${item.weight} kg</li>`).join('')}
          </ul>
          <p>Valor Total: $${optimalSolution.bestValue}</p>
          <p>Peso Total: ${calculateTotalWeight(optimalSolution.bestCombination)} kg</p>
        `;
      }
      highlightSelectedItems();
    }
      ...
      switch(algorithm) {
        ...
        case 'fptas':
          ...
          visualStepsDiv.innerHTML = `
            ...
            <p>3. Reconstrução da solução a partir da tabela DP.</p>
            ...
            <div class="explanation-step">
              ...
              <div class="visual-example">
                ...
                <ol>
                  ...
                  <li>Se já estiver no cache, retorna o resultado imediatamente (acesso em O(1))</li>
...
memo[key] = resultado;
      ...
      explanationTitle.textContent = `Explicação: ${title}`;
